...
if [ -z "$1" ]; then
    echo "Error: No argument provided. Please provide one of the following: onos, osc, oaic"
    exit 1
fi
...
case $1 in
    ...
    osc)
        ...
        ;;
    oaic)
        URLS_FILE="oaic_links.txt"
        ;;
    *)
        echo "Error: Invalid argument. Please provide one of the following: onos, osc, oaic"
        exit 1
        ;;
esac
